...
pipe = AutoPipelineForInpainting.from_pretrained("stabilityai/stable-diffusion-2-inpainting", torch_dtype=torch.float16, variant="fp16").to(device)
...
def predict(dict, prompt="", negative_prompt="", guidance_scale=7.5, steps=20, strength=1.0, scheduler="EulerDiscreteScheduler"):
    ...
    pipe.scheduler = scheduler.from_pretrained("stabilityai/stable-diffusion-2-inpainting", subfolder="scheduler", **add_kwargs)
                ...
                with gr.Column():
                    ...
                    with gr.Accordion(label="Advanced Settings", open=False):
                        with gr.Row(mobile_collapse=False, equal_height=True):
                            ...
                            steps = gr.Number(value=20, minimum=10, maximum=98, step=1, label="steps")
