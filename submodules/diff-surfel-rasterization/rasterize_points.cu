	...
	const bool debug)
{
  ...
  torch::Tensor out_semantics = torch::full({SEM_CHANNELS, H, W}, 0.0, float_opts);
  ...
  return std::make_tuple(rendered, out_color, out_semantics, out_others, radii, geomBuffer, binningBuffer, imgBuffer);
}
	...
	const bool debug) 
{
  ...
  torch::Tensor dL_dsemantics = torch::zeros({P, SEM_CHANNELS}, means3D.options());
  ...
  return std::make_tuple(dL_dmeans2D, dL_dcolors, dL_dsemantics, dL_dopacity, dL_dmeans3D, dL_dtransMat, dL_dsh, dL_dscales, dL_drotations);
}
